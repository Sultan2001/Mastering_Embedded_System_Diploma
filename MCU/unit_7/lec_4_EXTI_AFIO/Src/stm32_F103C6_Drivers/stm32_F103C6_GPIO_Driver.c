 ...
 * @retval 		- the Input Pin value (two value based on @ref GPIO_PIN_State_define)
 * Note			- none
 */
uint_8 MCAL_GPIO_ReadPin (GPIO_t * GPIOx , uint_16 pinNumber)
{
	uint_8 logic=0;
	if((GPIOx->IDR & pinNumber) != (uint_32) GPIO_PIN_State_RESET)
	{
		logic =GPIO_PIN_State_SET;
	}
	else
	{
		logic =GPIO_PIN_State_RESET;
	}
 ...
 * @param [in]	- GPIOx : where x can be (A..E depending on device used) to select the GPIO peripheral
 * @param [in] 	- pinNumber: Set Pin Number according to @ref GPIO_PIN_define
 * @param [in]   - value : the Output Pin value (two value based on @ref GPIO_PIN_State_define)
 * @retval 		- none
 * Note			- none
 */
