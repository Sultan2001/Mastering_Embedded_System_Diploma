...
#ifndef STM32_F103C6_DRIVERS_INC_STM32_F103C6_EXTI_DRIVER_H_
#define STM32_F103C6_DRIVERS_INC_STM32_F103C6_EXTI_DRIVER_H_
...
typedef struct
{
	uint_16 	EXTI_InputLineNumber;
	GPIO_t 	* 	GPIO_Port;
	uint_16 	GPIO_Pin;
	uint_8 		IVT_TRQ_Number;
...
typedef struct
{
	EXTI_GPIO_Mapping_t EXTI_Pin;	//specified the External interrupt GPIO mapping
									// this parameter must be set based on @ref EXTI_define
	uint_8 Trigger_Case;		    // specifies RISIG or Falling or Both trigger
									// this parameter must be set based on @ref EXTI_Trigger_define
	uint_8 IRQ_EN;					// Enable Or Disable EXTI IRQ
									// this parameter must be set based on @ref EXTI_IRQ_define
	void (*P_IRQ_CallBack)(void);	//set the C function() which will be call once IRQ happen
}EXTI_PinConfig_t;
...
#endif /* STM32_F103C6_DRIVERS_INC_STM32_F103C6_EXTI_DRIVER_H_ */
